...
Vue.component('model-form', {
    ...
    methods: {
        ...
        prepareClassElements (meta, data) {
            ...
            const elements = [];
            const groupMap = this.disableGroups ? {} : Jam.ArrayHelper.index('name', meta.groups);
        ...
        setErrors (data) {
            data = Jam.Helper.parseJson(data) || data || 'Error';
            this.setAttrErrors(data);
            if (typeof data === 'object') {
                data = Object.values(data)[0];
            }
            this.error = Jam.t(data);
        },
        setAttrErrors (data) {
            for (const ref of this.getRefElements()) {
                if (data.hasOwnProperty(ref.name)) {
                    ref.setError(data[ref.name]);
                    delete data[ref.name];
                }
            }
        },
        serialize () {
